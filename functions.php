...
// Homepage - index.php
	...
	function laadProducten(){
		...
		if(filter_input(INPUT_GET, 'categorie') != ""){
			...
			$groupQuery = "SELECT StockItemName, StockItemID, MarketingComments, StockGroupName, StockGroupID, UnitPrice, Photo FROM stockitems JOIN stockitemstockgroups USING(StockItemID) JOIN stockgroups USING(StockGroupID) WHERE StockGroupID = ".$groupID." GROUP BY StockItemID";
			// Sla de naam van de groep op uit de GET
			$groupName = filter_input(INPUT_GET, 'naamCategorie');
		}else{
			// In het geval dat er geen categorie is geselecteerd alle producten laden
			$groupQuery = "SELECT StockItemName, StockItemID, MarketingComments, StockGroupID, UnitPrice, Photo FROM stockitems JOIN stockitemstockgroups USING(StockItemID) GROUP BY StockItemID";
		}
...
// Subfunctions Artikelpagina
	...
	function AZ_homepage(){
		// Verbinden met database
		include("connect.php");
		// Verkrijg de zoekterm
		$searchID = filter_input(INPUT_GET, 'search');
		// Zoek in de database naar producten die de zoekterm in de naam hebben
		$searchQuery = "SELECT StockItemName, StockItemID, MarketingComments, UnitPrice, StockGroupID, Photo FROM stockitems JOIN stockitemstockgroups USING(StockItemID) ORDER BY StockItemName ASC ";
